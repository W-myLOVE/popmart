<template>
 <div class="tabcon1">
   <div class="en_title">PROSPECTUS</div>
   <div class="cn_title">招股文件</div>
   <div class="list_con">
     <div class="file_item"
          v-for="(item, index) in file1" :key="index">
       <div class="icon"></div>
       <div class="right">
         <div class="file_name">{{item.fileName}}</div>
         <div class="file_time">{{item.fileTime}}</div>
       </div>
     </div>
     <div class="block">
       <div class="buttonleft">&lt;</div>
       <div class="number">1</div>
       <div class="buttonRight">&gt;</div>
     </div>
   </div>
 </div>
</template>

<script>
export default {
  data () {
    return {
      file1:[
        {fileName:'招股书.pdf', fileTime:'2020-12-01 18:01:36'},
        {fileName:'全球发售正式通告.pdf', fileTime:'2020-12-01 18:01:36'},
        {fileName:'绿色申请表格.pdf', fileTime:'2020-12-01 18:01:36'}
      ]
    }
  }
}

</script>
<style scoped>
.tabcon1{
  width: 64.063vw;
  margin: 0 auto;
  padding: 4.67vw 0;
}
.en_title{
  text-align: left;
  font-size: 2.604vw;
  color: #1a1a1a;
  font-weight: 900;
  line-height: 2.604vw;
  margin-bottom: .938vw;
}
.cn_title{
  text-align: left;
  font-size: 1.563vw;
  line-height: 1.563vw;
  color: #000;
  margin-bottom: 4.167vw;
}
.file_item{
  display: flex;
  justify-content: left;
  width: 62.5vw;
  border-bottom: .104vw solid #b7b7b7;
  cursor: pointer;
  margin-bottom: 1.042vw;
}
.icon{
  width: 2.552vw;
  height: 1.198vw;
  background: url(data:image/png;base64,) no-repeat;
  background-size: 100%;
  margin-top: .417vw;
  margin-right: 1.458vw;
}
.file_name{
  font-size: 1.25vw;
  font-weight: 600;
  margin-bottom: 1.042vw;
}
.file_time{
  font-size: 1.042vw;
  color: #e6e6e6;
  margin-bottom: 1.042vw;
}
.block{
  width: 100%;
  margin-bottom: .521vw;
  margin-top: 1.563vw;
  display: flex;
  justify-content: center;
}
.number{
  color: #333;
  padding: 0 28px;
  font-size: 13px;
  margin-top: 3px;
  font-weight: 700;
}
</style>
